...
class InputValor extends Component {
            ...
            <div className = "Input-Valor flex input-valor-open">
                <div className = "flex vertical">
                    <input  
                        ...
                        type = {this.props.type || "texto"} />
                    {erro && (<small className = "small-danger"> {erro} </small>)}
                </div>
                <div className = "flex flex-center">
                    <ButtonSimples 
                        type = "botão de sucesso pequeno" 
                        onClick = {() => this.handleSubmit (valor)}
                        rótulo = {(<i className = "fas fa-check" />)} />
                    <ButtonSimples 
                        type = "botão de perigo pequeno" 
                        onClick = {this.toggleForm}
                        rótulo = {(<i className = "fas fa-times" />)} />
                </div>
            </div>
        )
    }
    ...
    renderValue () {
        const {valor, erro} = this.props;
        return (
            <div className = "flex vertical">
                <div className = "Input-Valor flex" onClick = {() => this.toggleForm ()}>
                    <span className = {this.props.noStyle? "input-nostyle": "input"}> {valor} </span>
                    <div className = "flex flex-center">
                        <ButtonSimples 
                            type = "botão de aviso pequeno" 
                            rótulo = {(<i className = "fas fa-edit" />)} />
                    </div>
                </div>
                {erro && (<small className = "small-danger"> {erro} </small>)}
            </div>
