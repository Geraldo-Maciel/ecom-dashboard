...
export const getUser = () => {
    return function(dispatch) {
        ...
        .catch((erro) => console.log (erro, erro.response, erro.response && erro.response.data));
    }
}
...
export const updateUser = (dados, cb) => {
    return function(dispatch) {
        ...
        .catch((erro) => cb(errorHandling(erro)));
    }
}
...
export const transformeDate = (data, divisor, formato) => { 
    ...
    return moment(new data(ano,mes,dia)).format(formato);
}
...
export const updateUser = (dados, cb) => {
    função de retorno (despacho) {
        axios.put (`$ {api} / $ {versao} / api / usuarios /`, dados, getHeaders ())
        .então ((resposta) => {
            saveToken (response.data.usuario, true);
            despacho ({tipo: LOGIN_USER, carga útil: resposta.data});
            cb (nulo);
        })
        .catch ((erro) => cb (errorHandling (erro)));
    }
...
export const formatMoney = (valor) => {
    retornar `R $ {valor.toFixed (2) .split (". "). join (", ")}`
}

export const transformeDate = (data, divisor, formato) => { 
    const _data = data.split (divisor);
    const dia = Number (_data [0]) + 1;
    const mes = Número (_dados [1]) - 1;
    const ano = Número (_dados [2]);
    momento de retorno (nova data (ano, mes, dia)). formato (formato);
*/